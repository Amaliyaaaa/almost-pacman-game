...
class Enemy(GameSprite):
    def __init__(self, enemy_image, enemy_x, enemy_y, size_x, size_y, enemy_x_speed):
    # Вызываем конструктор класса (Sprite):
...
class Bullet(GameSprite):
    def __init__(self, bullet_image, bullet_x, bullet_y, size_x, size_y, bullet_x_speed):
    # Вызываем конструктор класса (Sprite):
...
monster = Enemy('truemonster.png', win_width - 80, 180, 80, 80, 6)
final_sprite = GameSprite('hearts.png', win_width - 85, win_height - 100, 80, 80)
...
while run:
 ...
 if not finish:
   ...
   if sprite.collide_rect(packman, monster):
       finish = True
       #вычисляем отношение
       img = image.load('truegameover.png')
